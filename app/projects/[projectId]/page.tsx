...
export default function ProjectShowPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 rounded-lg bg-gray-50 p-8 shadow-md">
      ...
      {/* Hitos de Pago */}
      <div className="space-y-4 rounded-lg bg-white p-6 shadow-lg">
        <h2 className="text-grey-900 text-2xl font-bold">Hitos de Pago del Proyecto</h2>
        <table className="min-w-full table-auto border-collapse border border-gray-200">
          <thead className="bg-indigo-500 text-white">
            <tr>
              {['Número de Hito', 'Valor', 'Descripción', ' '].map((header) => (
          ...
          <tbody>
            {project.hitos_pago_proyecto.map((item: any, index: number) => (
              <tr
                key={item.id}
                className={`text-center ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} hover:bg-gray-200`}
              >
                <td className="px-4 py-3">{item.numero_hito}</td>
                <td className="px-4 py-3">
                  {item.valor_hito ? formatCurrency(item.valor_hito) : ' '}
                </td>
                <td className="px-4 py-3">{item.descripcion_hito || ' '}</td>
                <td className="w-24">
      ...
      {/* Pagos Contratistas */}
      <div className="space-y-4 rounded-lg bg-white p-6 shadow-lg">
        <h2 className="text-grey-900 text-2xl font-bold">Pagos Contratistas</h2>
        <table className="min-w-full table-auto border-collapse border border-gray-200">
          <thead className="bg-indigo-500 text-white">
            <tr>
              {['Tipo', 'Instalador', 'Valor', 'Descripción', ' '].map((header) => (
                <th key={header} className="border border-gray-300 px-4 py-2">
                  {header}
                </th>
              ))}
          ...
          <tbody>
            {project.pago_contratistas.map((item: any, index: number) => (
              <tr
                key={item.id}
                className={`text-center ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'} hover:bg-gray-200`}
              >
                <td className="px-4 py-3">{item.tipo_pago || ''}</td>
                <td className="px-4 py-3">{item.instalador_name}</td>
                <td className="px-4 py-3">
                  {item.valor_pago ? formatCurrency(item.valor_pago) : ' '}
                </td>
                <td className="px-4 py-3">{item.descripcion_pago || ' '}</td>
                <td className="w-24">
